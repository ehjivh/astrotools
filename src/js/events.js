...
const H0SUN = -0.833;
const H0STAR = -0.583; // standard altitudes for rise and set
...
function findEvents(obj, jday, obs) {
	...
	if (obj == MOON) { // reference horizon h0 for Moon depends on parallax, see Meeus p. 102
		...
		let par = asind(6378.14 / bodies[obj].dist);
		h0moon = 0.7275 * par - 0.567;
	}
	let href0 = ((obj == SUN) ? H0SUN : H0STAR); // rise/set altitude depends on object
	if (obj == MOON) href0 = h0moon;

	let events = []; // stores the various events in records of [t, type]
	let count = 0;
	// find situation at start of interval (not currently used by AstroTools but needed by Skyplanner)
	bodies[obj].update(jday, obs);
	let altaz = radec2aa(bodies[obj].ra, bodies[obj].dec, jday, obs);
	let alt = altaz[0];
	let type = 4;
	...
	bodies[obj].update(jday + 0.5, obs);
	let dec1 = bodies[obj].dec;
	altaz = radec2aa(bodies[obj].ra, bodies[obj].dec, jday + 0.5, obs);
	let H = altaz[2]; // H is hour angle
	let m = -H / 360.0; // first transit approx.
	let m0;
	for (let i = 0; i < 2; i++) { // check for events around first and second transit
		bodies[obj].update(jday + 0.5 + m, obs);
		let altaz = radec2aa(bodies[obj].ra, bodies[obj].dec, jday + 0.5 + m, obs);
		let H = altaz[2] > 180.0 ? altaz[2] - 360 : altaz[2];
		m0 = m - H / 360.0; // correction to transit time (Meeus page 103)
		if (m0 >= -0.5 && m0 < 0.5)
			events[count++] = [m0 + 0.5, 0]; // save transit time
		// find rise and set times (and start/end of twilights if sun)
		for (let j = 0; j <= (obj == SUN ? 3 : 0); j++) {
			let href = -6.0 * j; // href is the desired reference horizon
			if (href == 0.0) {
				href = href0;
			}
			let cosH0 = (sind(href) - sind(obs.latitude) * sind(dec1)) / (cosd(obs.latitude) * cosd(dec1));
			// (Meeus 15.1)
			if (cosH0 >= -1.0 && cosH0 <= 1.0) {
				// this may miss occasional rises/sets in polar regions, especially for Moon
				let H0 = acosd(cosH0);

				let m1 = m0 - H0 / 360.0; // rise (Meeus 15.2)
				bodies[obj].update(jday + 0.5 + m1, obs);
				let altaz = radec2aa(bodies[obj].ra, bodies[obj].dec, jday + 0.5 + m1, obs);
				H = altaz[2];
				...
				if (m1 >= -0.5 && m1 < 0.5) { // only keep event within interval of interest
					events[count++] = [m1 + 0.5, -j - 1];
				}

				let m2 = m0 + H0 / 360.0; // set
				bodies[obj].update(jday + 0.5 + m2, obs);
				altaz = radec2aa(bodies[obj].ra, bodies[obj].dec, jday + 0.5 + m2, obs);
				H = altaz[2];
				// correction to set time
				m2 += (altaz[0] - href) / (360 * cosd(bodies[obj].dec) * cosd(obs.latitude) * sind(H));
				if (m2 >= -0.5 && m2 < 0.5) {
					events[count++] = [m2 + 0.5, j + 1];
				}
			}
		}
		m += 1.0; // second transit approx.
	}
	events[count++] = [1.0, -9]; // end marker
