...
function rewrite2() {
	...
	let jdstr = fixnum(jd(observer), 11, 3);
...
function jd2dt() {
	...
	let zdt = jdtocd(parseFloat(tbl.julian.value) - observer.tz / 1440.0);
...
function updateobject(cat, deepsky) {
	...
	if (cat) {
		let objlist = (deepsky ? tbl.deepskyobj : tbl.fixstar);
		let ndx = objlist.selectedIndex - 1;
...
function switchboard(funct) {
	...
	let starttime = new Date(); // if timer_on: Measure execution time
	let dspan = datecount[tbl.Times.selectedIndex];
	let dstep = daystep[tbl.Step.selectedIndex];
	...
	let chart = tbl.outputtype[0].checked;
	let deepsky = (tbl.Starordso.selectedIndex == 1);
	let p = objlist[tbl.object.selectedIndex];
	if (p == USER) {
