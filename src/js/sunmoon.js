...
function sunrise(obs, twilight) {
	...
	obscopy.minutes = 0;
	obscopy.seconds = 0;
	let riseset = [0.0, 0.0, false, 0.0, 0.0];
	let lst = local_sidereal(obscopy);
	let jday = jd(obscopy);
	let radec = SunAlt(jday, obscopy);
	let ra = radec[3];
	let dec = radec[4];
	let UTsun = 12.0 + ra / 15.0 - lst;
	if (UTsun < 0.0) UTsun += 24.0;
	if (UTsun > 24.0) UTsun -= 24.0;
	let cosLHA = (sind(twilight) - sind(obs.latitude) * sind(dec)) / (cosd(obs.latitude) * cosd(dec));
	...
	if (cosLHA <= 1.0 && cosLHA >= -1.0) {
		...
		riseset[2] = true;
		let lha = acosd(cosLHA) / 15.0;
		let rtime;
		if ((UTsun - lha) < 0.0) {
			rtime = 24.0 + UTsun - lha;
		} else {
			rtime = UTsun - lha;
		}
...
function MoonPos(jday, obs) {
	...
	let T = (jday - 2451545.0) / 36525;
	// Moons mean longitude L'
	let LP = rev(218.3164477 + 481267.88123421 * T);
	// Moons mean elongation
	let D = rev(297.8501921 + 445267.1114034 * T);
	// Suns mean anomaly
	let M = rev(357.5291092 + 35999.0502909 * T);
	// Moons mean anomaly M'
	let MP = rev(134.9633964 + 477198.8675055 * T);
	// Moons argument of latitude
	let F = rev(93.2720950 + 483202.0175233 * T);
	...
	let Sl = 6288774 * sind(MP) + 1274027 * sind(2 * D - MP) + 658314 * sind(2 * D) + 213618 * sind(2 * MP) -
	...
	let Sb = 5128122 * sind(F) + 280602 * sind(MP + F) + 277602 * sind(MP - F) + 173237 * sind(2 * D - F) +
		...
		4200 * sind(2 * D + MP + F);
	let Sr = (-20905355) * cosd(MP) - 3699111 * cosd(2 * D - MP) - 2955968 * cosd(2 * D) - 569925 * cosd(2 * MP) +
	...
	let mglong = rev(LP + Sl / 1000000.0);
	let mglat = Sb / 1000000.0;
	// Obliquity of Ecliptic
	let obl = 23.4393 - 3.563E-7 * (jday - 2451543.5);
	let ra = rev(atan2d(sind(mglong) * cosd(obl) - tand(mglat) * sind(obl), cosd(mglong)));
	let dec = asind(sind(mglat) * cosd(obl) + cosd(mglat) * sind(obl) * sind(mglong));
	let moondat = radec2aa(ra, dec, jday, obs);
	// phase angle (48.4)
	let pa = Math.abs(180.0 - D - 6.289 * sind(MP) + 2.100 * sind(M) - 1.274 * sind(2 * D - MP) -
		0.658 * sind(2 * D) - 0.214 * sind(2 * MP) - 0.11 * sind(D));
	let k = (1 + cosd(pa)) / 2;
	let mr = Math.round(385000.56 + Sr / 1000.0);
	let h = moondat[0];
	...
	let sdat = sunxyz(jday);
	let r = sdat[3]; // Earth (= Moon) distance to Sun in AU
	let R = mr / 149598000; // Moon distance to Earth in AU
	let mag = 0.23 + 5 * log10(r * R) + 0.026 * pa + 4.0E-9 * pa * pa * pa * pa
	return [h, moondat[1], moondat[2], ra, dec, mglong, mglat, k, r, mr, mag];
} // Moonpos()