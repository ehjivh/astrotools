...
let c_index = -1; // currently selected comet
...
const comets = []; // orbital elements in internal format
...
function comet_xyz(cn, jday) {
	...
	let q = cm.q;
	let e = cm.e;
	let Tj = jd0(cm.Ty, cm.Tm, cm.Td); // get julian day of perihelion time
	let r;
	let v;
	if (e > 0.98) {
		// treat as near parabolic (approx. method valid inside orbit of Pluto)
		let k = 0.01720209895; // Gaussian gravitational constant
		let a = 0.75 * (jday - Tj) * k * Math.sqrt((1 + e) / (q * q * q));
		let b = Math.sqrt(1 + a * a);
		let W = cbrt(b + a) - cbrt(b - a);
		let c = 1 + 1 / (W * W);
		let f = (1 - e) / (1 + e);
		let g = f / (c * c);
		let a1 = (2 / 3) + (2 / 5) * W * W;
		let a2 = (7 / 5) + (33 / 35) * W * W + (37 / 175) * W * W * W * W;
		let a3 = W * W * ((432 / 175) + (956 / 1125) * W * W + (84 / 1575) * W * W * W * W);
		let w = W * (1 + g * c * (a1 + a2 * g + a3 * g * g));
		v = 2 * atand(w);
		r = q * (1 + w * w) / (1 + w * w * f);
	} else { // treat as elliptic
		let a = q / (1.0 - e);
		let P = 365.2568984 * Math.sqrt(a * a * a); // period in days
		let M = 360.0 * (jday - Tj) / P; // mean anomaly
		// eccentric anomaly E
	...
	let N = cm.N + 3.82394E-5 * d; // precess from J2000.0 to now;
	let w = cm.w; // why not precess this value?
	let i = cm.i;
...
function CometAlt(jday, obs) {
	...
	let planet_xyz = comet_xyz(cn, jday);
	...
	let dist = radec[2];
	let r = planet_xyz[3];
	let mag = comets[cn].G + 5 * log10(dist) + 2.5 * comets[cn].H * log10(r); // Schlyter's formula is wrong!
	return [altaz[0], altaz[1], altaz[2], ra, dec, lon, lat, 1.0, r, dist, mag];
} // CometAlt()